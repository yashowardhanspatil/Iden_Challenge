...
OUTPUT_JSON = "updated_products.json"
...
async def extract_products(page):
    ...
    for i, container in enumerate(product_containers):
        try:
            ...
            title_element = await container.query_selector("div.h-12.text-white")
            if title_element:
                product["Title"] = await title_element.inner_text()

            labels = await container.query_selector_all("span.text-muted-foreground")
            ...
            if not product.get("ID") and len(values) >= 4:
